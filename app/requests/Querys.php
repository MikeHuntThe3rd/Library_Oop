...
class Querys{
    ...
    public function SingleRow($id){
        return $this->db->SingleQuery("SELECT ISBN, name, img, writer, lang, price, publisher.publisher, genre.genre FROM `books` 
        JOIN genre ON genre.id = genre_id 
        JOIN publisher ON publisher.id = publisher_id WHERE ISBN=". $id);
    }
